...
PROJECT_ROOT_DIR=$(cd ${CURRENT_DIR}/../.. && pwd)
...
TEST_DIRS=( "${CURRENT_DIR}/cases/app" \
            "${CURRENT_DIR}/cases/basic" \
            "${CURRENT_DIR}/cases/complex" \
            "${CURRENT_DIR}/cases/cpp_integration" \
            "${CURRENT_DIR}/cases/runtime" \
            "${CURRENT_DIR}/cases/cpp_closures")
...
if [ "$OSTYPE" == "msys" ]; then
    EXCLUDE_FILTER+=("exceptions.ts")   # FIXME: TSN-65
    EXCLUDE_FILTER+=("date.ts")         # FIXME: TSN-163
    EXCLUDE_FILTER+=("inher.ts")        # FIXME: TSN-164
    EXCLUDE_FILTER+=("runtime.ts")      # FIXME: TSN-165
fi
...
run_tests() {
    for dir in ${TEST_DIRS[@]}; do
        ...
        tests=$(/usr/bin/find ${dir} -name "${INCLUDE_FILTER}" ${EXCLUDE_EXPR})
        ...
        for i in $(/usr/bin/find ${test_out_dir} -name CTestTestfile.cmake); do
...
build_test() {
    ...
    tsnative.sh \
        ...
        --conan_install ${PROJECT_ROOT_DIR} \
        ${EXT_OPT} \
        --source $1 \
        --build ${build_dir} \
