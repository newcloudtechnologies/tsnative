...
class LLVMConan(ConanFile):
    ...
    url = 'https://github.com/conan-io/conan-center-index/recipes/llvm-core'
    ...
    default_options = {
        ...
        'fPIC': True,
        'components': 'all',
        'targets': 'AArch64;ARM;RISCV;X86', # 'all
    ...
    def _configure_cmake(self):
        ...
        if self.settings.os == 'Windows' and self.settings.compiler == 'Visual Studio':
            # Limit number of parallel jobs for msvc to be able to compile this
            # recipe on machines with 16gb of RAM
            cmake.definitions['CMAKE_BUILD_PARALLEL_LEVEL'] = '2'

        cmake.definitions['BUILD_SHARED_LIBS'] = self.options.shared # False
        ...
        cmake.definitions['LLVM_ENABLE_BINDINGS'] = 'OFF' # False
        cmake.definitions['LLVM_CCACHE_BUILD'] = False

        cmake.definitions['LLVM_INCLUDE_TOOLS'] = self.options.shared
        cmake.definitions['LLVM_INCLUDE_EXAMPLES'] = self.options.build_examples # False
        cmake.definitions['LLVM_INCLUDE_TESTS'] = self.options.build_tests # False
        ...
        # nct
        cmake.definitions['LLVM_ENABLE_PROJECTS'] = self.options.enable_projects
        cmake.definitions['CMAKE_BUILD_TYPE'] = self.settings.build_type
        cmake.definitions['LLVM_INCLUDE_TOOLS'] = self.options.build_tools
        cmake.definitions['LLVM_BUILD_TOOLS'] = self.options.build_tools
        cmake.definitions['LLVM_BUILD_EXAMPLES'] = self.options.build_examples
        cmake.definitions['LLVM_BUILD_TESTS'] = self.options.build_tests
        cmake.definitions['CMAKE_CXX_FLAGS'] = '-std=c++11'

        self.output.info("command_line: {}".format(cmake.command_line))
    ...
    def source(self):
        git = Git(self)
        clone_args = [self._source_subfolder, '--depth', '1', '--branch', 'llvmorg-{}'.format(self.version)]
        git.clone(url="https://github.com/llvm/llvm-project.git", args=clone_args)
        self._patch_sources()
    ...
    # Helpers for local builds on windows
    # Never called by default. Modify build() method to call them
    def build_with_msvc(self):
        self.output.info("Building with MSVC")
        llvm_dir = os.path.join(self.source_folder, "source_subfolder", "llvm")

        # TODO: remove -DCMAKE_INSTALL_PREFIX
        command = ('cmake -G "Visual Studio 16 2019" -A x64 -Thost=x64 -DCMAKE_BUILD_TYPE:STRING={} -DLLVM_TARGETS_TO_BUILD="{}" '
        '-DLLVM_ENABLE_PROJECTS="{}" -DCMAKE_CXX_STANDARD:STRING=17 -DLLVM_BUILD_TOOLS={} '
        '-DLLVM_STATIC_LINK_CXX_STDLIB:BOOL=ON -DLLVM_INCLUDE_EXAMPLES={} -DLLVM_INCLUDE_TESTS={} '
        '-DLLVM_ENABLE_EH={} -DLLVM_ENABLE_RTTI={} '   #  -DCMAKE_INSTALL_PREFIX="{}"
        '-DBUILD_SHARED_LIBS={} {}').format(self.settings.build_type, self.options.targets, self.options.enable_projects, 
        self.options.build_tools, self.options.build_examples, self.options.build_tests, self.options.exceptions, self.options.rtti,
        self.options.shared, llvm_dir) # self.package_folder, 

        self.output.info("command: {}".format(command))

        self.run(command, win_bash=True)
        self.run("cmake --build . --config Release", win_bash=True)

    def build_with_MinGW(self):
        self.output.info("Building with MinGW")

        # llvm_dir = '{}/source_subfolder/llvm'.format(self.source_folder)

        llvm_dir = os.path.join(self.source_folder, "source_subfolder/llvm")

        command = ('cmake -G "Ninja" -DCMAKE_BUILD_TYPE:STRING={} -DLLVM_TARGETS_TO_BUILD="{}" '
        '-DLLVM_ENABLE_PROJECTS="{}" -DLLVM_BUILD_TOOLS={} -DLLVM_INCLUDE_EXAMPLES={} '
        '-DLLVM_INCLUDE_TESTS={} -DLLVM_INCLUDE_BENCHMARKS=OFF -DLLVM_BUILD_BENCHMARKS=OFF '
        '-DLLVM_ENABLE_EH={} -DLLVM_ENABLE_RTTI={} -DCMAKE_CXX_FLAGS=-std=c++11 '
        '-DCMAKE_EXE_LINKER_FLAGS="-static-libgcc -static-libstdc++ -static -lstdc++ -lpthread" -DBUILD_SHARED_LIBS={} {}').format(
            self.settings.build_type, self.options.targets, self.options.enable_projects, self.options.build_tools, 
            self.options.build_examples, self.options.build_tests, self.options.exceptions, self.options.rtti, self.options.shared, llvm_dir)
