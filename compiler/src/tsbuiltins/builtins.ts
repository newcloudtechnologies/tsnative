...
class Builtin {
  ...
  private init(): void {
    const clazz = this.generator.symbolTable.get(this.name) as Scope;
    if (!clazz.symbol) {
      throw new Error(`Class '${this.name}' have no stored symbol`);
    }

    this.thisData = this.generator.meta.getThisData(clazz.symbol);
  }
}
...
export class BuiltinTSClosure extends Builtin {
  ...
  private initClassDeclaration() {
    ...
    const classDeclaration = defs.statements.find((node) => {
      return (
        ts.isClassDeclaration(node) && node.name?.getText() === "TSClosure"
      );
    });

    if (!classDeclaration) {
      throw new Error(
        "Unable to find 'TSClosure' declaration in std library utility definitions"
      );
    }
  ...
  private initCallFn() {
    ...
    const callDeclaration = this.declaration.members.find(
      (m) => m.isMethod() && m.name?.getText() === "call"
    );
    ...
    const { fn: call } = this.generator.llvm.function.create(
      llvmReturnType,
      llvmArgumentTypes,
      qualifiedName
    );
  ...
  private initGetEnvironmentFn() {
    ...
    if (!getEnvironmentDeclaration) {
      throw new Error(
        "No function declaration for TSClosure.getEnvironment provided"
      );
    }
    ...
    const llvmReturnType = LLVMType.getInt8Type(this.generator)
      .getPointer()
      .getPointer()
      .getPointer();
    const llvmArgumentTypes = [this.getLLVMType()];
  ...
  private initCtorFn() {
    ...
    const constructorDeclaration = this.declaration.members.find((m) =>
      m.isConstructor()
    );
    if (!constructorDeclaration) {
      throw new Error(
        `Unable to find constructor declaration at '${this.declaration.getText()}'`
      );
    }
    ...
    const llvmArgumentTypes = [
      ...
      LLVMType.getInt8Type(this.generator)
        .getPointer()
        .getPointer()
        .getPointer(),
  ...
  createClosure(
    ...
    if (
      fn.type.getPointerLevel() !== 1 ||
      !fn.type.unwrapPointer().isFunction()
    ) {
    ...
    const thisValue = this.generator.gc.allocateObject(
      this.getLLVMType().unwrapPointer()
    );
    ...
    if (numArgs > 63) {
      throw new Error(
        `Parameters limited up to 63. Error at closure creation for '${functionDeclaration.getText()}'`
      );
    }

    const optionals = functionDeclaration.parameters.reduce(
      (acc, parameter, index) => {
    ...
    this.generator.builder.createSafeCall(constructor, [
      ...
      untypedFn,
      untypedEnv,
      this.generator.builtinNumber.create(
        LLVMConstantFP.get(this.generator, envLength)
      ),
      this.generator.builtinNumber.create(
...
export class BuiltinNumber extends Builtin {
  ...
  private initClassDeclaration() {
    ...
    const classDeclaration = stddefs.statements.find((node) => {
      return (
        ts.isClassDeclaration(node) && node.name?.getText(stddefs) === "Number"
      );
    });

    if (!classDeclaration) {
      throw new Error(
        "Unable to find 'Number' declaration in std library definitions"
      );
    }
  ...
  private initCtorFn() {
    ...
    if (!isExternalSymbol) {
      throw new Error(
        `Number constructor for '${thisType.toString()}' not found`
      );
    }
  ...
  private initToStringFn() {
    ...
    const fnDeclaration = declaration.members.find(
      (m) => m.name?.getText() === "toString"
    );
    if (!fnDeclaration) {
      throw new Error(
        `Unable to find method 'toString' at '${declaration.getText()}'`
      );
    }
    ...
    if (!isExternalSymbol) {
      throw new Error(
        `Number method 'toString' for '${thisType.toString()}' not found`
      );
    }
  ...
  private initUnboxFn() {
    ...
    const unboxedDeclaration = declaration.members.find(
      (m) => m.name?.getText() === "unboxed"
    )!;
    const thisType = this.getTSType();
    ...
    const { fn: unboxFn } = this.generator.llvm.function.create(
      llvmReturnType,
      llvmArgumentTypes,
      qualifiedName
    );
  ...
  private initCloneFn() {
    ...
    const equalsDeclaration = declaration.members.find(
      (m) => m.isMethod() && m.name?.getText() === "clone"
    )!;
    ...
    const { fn: clone } = this.generator.llvm.function.create(
      llvmReturnType,
      llvmArgumentTypes,
      qualifiedName
    );
  ...
  createMathFn(name: string, flags: OperationFlags = OperationFlags.Binary) {
    ...
    if (!fnDeclaration) {
      throw new Error(
        `Unable to find method '${name}' at '${declaration.getText()}'`
      );
    }
    ...
    if (!isExternalSymbol) {
      throw new Error(
        `Number method '${name}' for '${thisType.toString()}' not found`
      );
    }
  ...
  private getNumberGetterFn(functionName: string) {
    ...
    if (!fnDeclaration) {
      throw new Error(
        `Unable to find method '${functionName}' at '${declaration.getText()}'`
      );
    }
    ...
    if (!isExternalSymbol) {
      throw new Error(
        `Number method '${functionName}' for '${thisType.toString()}' not found`
      );
    }
  ...
  private initNumberConstants(
    ...
    const nullValue = LLVMConstant.createNullValue(
      this.llvmType,
      this.generator
    );
    ...
    const casted = this.generator.builder.createBitCast(
      instance,
      this.llvmType
    );
    this.generator.builder.createSafeStore(casted, globalConstant);

    this.generator.symbolTable.globalScope.set(
      globalScopeKey,
      globalConstant,
      false
    );
...
export class BuiltinBoolean extends Builtin {
  ...
  private initClassDeclaration() {
    ...
    const classDeclaration = stddefs.statements.find((node) => {
      return (
        ts.isClassDeclaration(node) && node.name?.getText(stddefs) === "Boolean"
      );
    });

    if (!classDeclaration) {
      throw new Error(
        "Unable to find 'Boolean' declaration in std library definitions"
      );
    }
  ...
  private initUnboxFn() {
    ...
    if (!isExternalSymbol) {
      throw new Error(
        `Boolean constructor for '${thisType.toString()}' not found`
      );
    }
  ...
  private initConstructorFn() {
    ...
    if (!constructorDeclaration) {
      throw new Error(
        `Unable to find constructor at '${this.classDeclaration.getText()}'`
      );
    }
  ...
  private initNegateFn() {
    ...
    const negateDeclaration = declaration.members.find(
      (m) => m.name?.getText() === "negate"
    )!;
    const thisType = this.getTSType();
    ...
    const { fn: negateFn } = this.generator.llvm.function.create(
      llvmReturnType,
      llvmArgumentTypes,
      qualifiedName
    );
  ...
  private initCloneFn() {
    ...
    const cloneDeclaration = declaration.members.find(
      (m) => m.isMethod() && m.name?.getText() === "clone"
    );

    if (!cloneDeclaration) {
      throw new Error(
        `Unable to find 'clone' method at '${this.classDeclaration.getText()}'`
      );
    }
...
export class BuiltinIteratorResult extends Builtin {
  ...
  private initValueGetter() {
    ...
    const valueGetterDeclaration = declaration.members.find(
      (m) => m.isGetAccessor() && m.name?.getText() === "value"
    );
    if (!valueGetterDeclaration) {
      throw new Error(
        `Unable to find 'value' getter in declaration: '${declaration.getText()}'`
      );
    }
    ...
    if (!isExternalSymbol) {
      throw new Error(
        `External symbol for 'value' is not found at '${declaration.getText()}'`
      );
    }
    ...
    const { fn: valueGetter } = this.generator.llvm.function.create(
      llvmReturnType,
      llvmArgumentTypes,
      qualifiedName
    );
  ...
  getDoneGetter(type: TSType) {
    ...
    const doneDeclaration = declaration.members.find(
      (m) => m.isGetAccessor() && m.name?.getText() === "done"
    )!;
    ...
    const { fn: doneGetter } = this.generator.llvm.function.create(
      llvmReturnType,
      llvmArgumentTypes,
      qualifiedName
    );
