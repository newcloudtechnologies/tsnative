...
export class ClassHandler extends AbstractNodeHandler {
  ...
  private handleClassDeclaration(declaration: Declaration, parentScope: Scope): void {
    ...
    this.generator.symbolTable.withLocalScope((localScope) => {
      ...
      const scope = new Scope(name, mangledTypename, this.generator.gc, false, parentScope, {
        declaration,
        llvmType,
        tsType: thisType,
        staticProperties,
      ...
      for (const type of clause.types) {
        ...
        if (typeArguments) {
          ...
          if (!parentScope.get(mangledTypename)) {
            ...
            const scope = new Scope(undefined, mangledTypename, this.generator.gc, false, parentScope, {
              declaration,
              llvmType,
              tsType: thisType,
              staticProperties,
