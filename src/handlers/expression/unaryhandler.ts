...
export class UnaryHandler extends AbstractExpressionHandler {
  ...
  private handlePrefixUnaryExpression(expression: ts.PrefixUnaryExpression): llvm.Value {
    const { operand } = expression;

    switch (expression.operator) {
      case ts.SyntaxKind.PlusToken:
        return this.generator.handleExpression(operand);
      case ts.SyntaxKind.MinusToken:
        return this.generator.builder.createFNeg(this.generator.handleExpression(operand));
      case ts.SyntaxKind.PlusPlusToken: {
        const value = this.generator.handleValueExpression(operand);
      ...
      case ts.SyntaxKind.TildeToken:
        return castToInt32AndBack([this.generator.handleExpression(operand)], this.generator, ([value]) =>
          this.generator.builder.createNot(value)
        );
      case ts.SyntaxKind.ExclamationToken:
        return this.generator.builder.createNot(
          makeBoolean(this.generator.handleExpression(operand), operand, this.generator)
        );
      default:
  ...
  private handlePostfixUnaryExpression(expression: ts.PostfixUnaryExpression): llvm.Value {
    const { operand } = expression;
